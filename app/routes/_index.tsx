...
export default function Index() {
	...
	return (
		<div className="max-w-2xl mx-auto py-8">
								...
								<div className="flex items-center gap-2">
									<Form method="post">
										...
										<button
											...
											value="toggle"
											className={
												todo.doneAt ? "text-green-600" : "text-gray-400"
											}
									...
									{todo.dueDate && (
										<span className="ml-2 text-xs text-gray-500">
											期限: {String(todo.dueDate).slice(0, 10)}
										</span>
									)}
									{todo.doneAt && (
										<span className="ml-2 text-xs text-green-600">
											完了: {String(todo.doneAt).slice(0, 10)}
										</span>
									)}
								</div>
								{todo.detail && (
			...
			{actionData?.error && (
				<div className="text-red-600 mt-4">{actionData.error}</div>
			)}
		</div>
	);
}
